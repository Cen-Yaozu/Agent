...
export function ErrorMessage({ error, showDetails = false }: ErrorMessageProps) {
  ...
  const textClass = config.text;

  // Subtype badge color
  const subtypeColors: Record<ErrorEvent["subtype"], string> = {
        ...
        <div className="pl-3 sm:pl-0">
          <div className={`text-sm ${config.bg} ${config.border} border rounded-lg p-3`}>
            ...
            <div className="flex items-center gap-2 mb-2">
              <span
                className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${subtypeColors[subtype]}`}
              >
                {subtype.toUpperCase()}
              </span>
              {error.code && (
